  ...
  for(col in colnames(m)) {
    ...
    rez <- sapply(subset[2:ncol(subset)], function(x) {
      if(any(is.na(x))) {
        ...
        ifelse(length(unique(x[!is.na(x)])) == 1, 1, 0)
      }
      })
...
m %>% 
  ...
  labs(x = NULL, y = NULL) +
  scale_fill_gradient2(low = "#80DEEA", mid = "#009688", high = "#43A047", midpoint = 0.5)
```
...
plot_dendrogram(model, clusters = clusters, ylim = -0.3, nudge_y = -0.02, angle = 90, points = FALSE, h = 0.6)
```
