...
```{r eval=TRUE, class.source = "fold-hide"}
plot_dendrogram <- function(model, clusters = NULL, points = TRUE, ylim = -0.3, nudge_y = -0.02, angle = 90, h = NULL, edges = "elbow") {
  if(is.null(clusters)) {
  ...
  if(points) {
    if(is.null(clusters)) {
      ...
      p <- p + geom_node_point(aes(color = cluster, filter=leaf))
    }
  }
