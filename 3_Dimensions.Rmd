...
<!-- pca <- parameters::principal_components(data[hrv_cols], n=1, rotation = "promax") -->
...
<!--              PCA_1 = .) %>%  -->
<!--   mutate(PCA_1 = as.character(PCA_1)) %>%  -->
<!--   atawizard::data_rename_rows() -->
<!-- ``` -->
...
table_clusters <- parameters::closest_component(pca) %>% 
  data.frame(Index = names(.),
             PCA_12 = .) %>%
  mutate(PCA_12 = as.character(PCA_12)) %>%
